document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    if (pendingReturns > 0) {
      showBanner(
        `<strong>Return requests waiting:</strong> ${pendingReturns} item${pendingReturns === 1 ? '' : 's'} currently need admin review. Open <strong>Returns</strong> to process them.`,
        'warning'
      );
    } else if (lowStockCount > 0) {
      showBanner(
        `<strong>Low stock warning:</strong> ${lowStockCount} product${lowStockCount === 1 ? '' : 's'} ${lowStockCount === 1 ? 'is' : 'are'} at or below the configured stock threshold.`,
        'warning'
      );
    } else {
    ...
    statsGrid.innerHTML = statCards.map(([label, value]) => {
      ...
      return `
        <div class="stat-card${alertCard ? ' stat-card-danger' : ''}">
          <h3>${escapeHtml(label)}</h3>
          <div class="stat-big">${escapeHtml(value)}</div>
        </div>
      `;
    }).join('');
          ...
          <tbody>
            ${orders.map(order => {
              const statusOptions = ['pending', 'paid', 'shipped', 'delivered', 'returned', 'cancelled']
                .map(status => { const label = status === 'pending' ? 'processing' : status; return `<option value="${status}" ${order.status === status ? 'selected' : ''}>${label}</option>`; })
                .join('');

              return `
                <tr>
                  <td>#${order.id}</td>
                  <td>${escapeHtml(order.user ? order.user.name : 'Unknown')}</td>
                  <td>${money(order.total)}</td>
                  <td><span class="status-pill status-${escapeHtml(order.status)}">${escapeHtml(order.status === 'pending' ? 'processing' : order.status)}</span></td>
                  <td>${formatDate(order.created_at)}</td>
                  <td>
                    <select data-order-id="${order.id}" class="order-status-select">${statusOptions}</select>
                    <button class="btn-ghost save-status" data-order-id="${order.id}">Save</button>
                  </td>
                </tr>
              `;
            }).join('')}
        ...
        <table>
          ...
          <tbody>
            ${lowStock.map(item => {
              ...
              return `
                <tr class="${rowClass}">
                  <td>${escapeHtml(item.variant && item.variant.product ? item.variant.product.name : 'Unknown product')}</td>
