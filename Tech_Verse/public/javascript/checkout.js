(function () {
  ...
  // Guard: verify user is signed in before allowing checkout
    ...
    placeBtn.addEventListener('click', async function () {
      ...
      try {
        const res = await fetch('/api/checkout', {
          ...
          headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': getCsrfToken() },
          body: JSON.stringify({
            items: keys.map(k => ({ variant_id: items[k].id, quantity: items[k].qty })),
        ...
        // Only clear cart after server confirms
